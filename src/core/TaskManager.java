...
/**
 * This class is responsible for main tasks operations that can be done to a
 * tasks Operations such as Add, Remove, Update and Sorting The ToDol is a
 * temporary container HashMap for doing the tasks operations on after reading
...
public class TaskManager {
	...
	/**
	 * Method for removing the task based on the TaskNumber It deletes the project
	 * also once there is no tasks in the project
	 * 
			...
			for (int i = 0; i < Itr.getValue().size(); i++) {
				...
				if (task.getTaskNumber() == TaskNumber) {
					...
					if (task.getTaskStatus().equalsIgnoreCase("Done"))
						System.out.println(
								"Task status has been updated  from " + task.getTaskStatus() + " ==> " + "ToDo ");
					else
						System.out.println(
								"Task status has been updated  from " + task.getTaskStatus() + " ==> " + " Done ");
	...
	/**
	 * Method to validate the user entry for the task number for editing purposes to
	 * make sure only exists TaskNumber chosen
	 * 
	 ...
	 * the HashMap for file operations purposes
	 * 
	 * @return
	 */
	public List<Task> MapToList() {
		return ToDol.values().stream().flatMap(Collection::stream).collect(Collectors.toCollection(ArrayList::new));
	}
